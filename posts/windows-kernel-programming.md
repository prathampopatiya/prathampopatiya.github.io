...
> **Note:** If you take a look at the exported functions list from NtOsKrnl.exe, you'll find many functions that are not documented in the Windows Driver Kit
...
> **Important:** Clearly, the non-paged pool is a "better" memory pool as it can never incur a page fault.
...
Pool tags are 4-byte identifiers used to identify memory allocations. They help with debugging and tracking memory usage. Some common tags include:

| Tag      | Purpose                          |
| -------- | -------------------------------- |
| `Tag1`   | Example tag for demonstration    |
| `POOL`   | Generic pool allocation          |
| `HEAP`   | Heap-related allocation          |

---

## Linked Lists

Linked lists are a fundamental data structure used extensively in kernel programming. The kernel provides the `LIST_ENTRY` structure for creating doubly-linked lists.
...
| `IRP_MJ_WRITE`                   | Write operation. Typically invoked for `WriteFile`, `ZwWriteFile`, and similar write APIs                        |
| `IRP_MJ_DEVICE_CONTROL`          | Device control operation. Invoked because of `DeviceIoControl` or `ZwDeviceIoControlFile` calls                 |
| `IRP_MJ_INTERNAL_DEVICE_CONTROL` | Generic call to a driver, invoked because of internal device control operations                                  |
...
The structure is typically initialized with the `InitializeObjectAttributes` macro, that allows specifying all the structure members except Length (set automatically by the macro), and `SecurityQualityOfService`, which is not normally needed.
...
Windows Kernel Programming requires careful attention to memory management, proper use of kernel APIs, and understanding of the structural components like driver objects and object attributes. Mastery of these fundamentals is essential for developing reliable and efficient kernel-mode drivers.